...
# falls back to a plain link test for libedit/check if it's missing)
# but gives the most reliable detection, so keep it.
...
# test suite (./configure only warns and disables tests if check is
